...
export default function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (navRef.current) {
        if (scrolled) {
          // Solid white glass effect on scroll
          gsap.to(navRef.current, {
            backgroundColor: 'rgba(255, 255, 255, 0.95)',
            backdropFilter: 'blur(12px)',
            borderBottomColor: 'rgba(229, 226, 223, 0.3)',
            boxShadow: '0 4px 12px rgba(0, 0, 0, 0.08)',
            duration: 0.3,
            overwrite: 'auto',
          });
        } else {
          // White glass effect at top
          gsap.to(navRef.current, {
            backgroundColor: 'rgba(255, 255, 255, 0.8)',
            backdropFilter: 'blur(10px)',
            borderBottomColor: 'rgba(229, 226, 223, 0.2)',
  ...
  const navLinks = [
    ...
    <nav
      ref={navRef}
      className="fixed top-0 left-0 right-0 z-50 h-[72px] flex items-center transition-all duration-300 border-b border-gray-200/30"
      style={{
        backgroundColor: 'rgba(255, 255, 255, 0.8)',
        backdropFilter: 'blur(10px)',
    ...
    >
      <div className="container w-full flex items-center justify-between h-full">
        ...
        <div className="flex items-center gap-4">
          {/* Wishlist */}
          <Link href="/favorites">
            <a className="relative inline-flex items-center p-2 hover:bg-gray-100 rounded-lg transition-colors duration-300 text-foreground/70 hover:text-foreground">
              <Heart size={20} />
              {favorites.length > 0 && (
                <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] leading-none px-1 rounded">{favorites.length}</span>
              )}
            </a>
          </Link>
          ...
          <button
            onClick={toggleTheme}
            className="p-2 hover:bg-gray-100 rounded-lg transition-colors duration-300 text-foreground/70 hover:text-foreground"
          >
          ...
          <button
            ...
            className="md:hidden p-2 hover:bg-gray-100 rounded-lg transition-colors duration-300 text-foreground/70 hover:text-foreground"
          >
